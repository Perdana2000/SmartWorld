...
class TFTUI(threading.Thread):
    ...
    Verbeteringen voor touch:
    - grotere hitbox (pad)
    - sneller debounce
    - mini-stabilize (2e sample en middelen)
    - wacht tot release (voorkomt dubbel taps)
    """
    ...
    def get_touch_raw(self, samples=5):
    ...
    # grotere "hitbox" zodat je niet super precies hoeft te tikken
    def hit(self, x, y, r, pad=35):
        x1, y1, x2, y2 = r
    ...
    def run(self):
        while self.running:
            ...
            raw = self.get_touch_raw(samples=5)
            if raw and (time.time() - self.last_press) > self.DEBOUNCE:
                self.last_press = time.time()
                rx, ry = raw

                # mini-stabilize: pak nog 1 sample en middel
                time.sleep(0.02)
                raw2 = self.get_touch_raw(samples=5)
                if raw2:
                    rx2, ry2 = raw2
                    rx = (rx + rx2) // 2
                    ry = (ry + ry2) // 2
                ...
                if st.get("screen") == "menu":
                    if self.hit(sx, sy, self.button_rect(0), pad=30):
                        self.c.goto("moisture")
                    elif self.hit(sx, sy, self.button_rect(1), pad=30):
                        self.c.goto("light")
                    elif self.hit(sx, sy, self.button_rect(2), pad=30):
                        self.c.goto("water")

                # PAGES
                else:
                    if hasattr(self, "back_rect") and self.hit(sx, sy, self.back_rect, pad=30):
                        self.c.goto("menu")

                    if st.get("screen") in ("moisture", "light"):
                        if hasattr(self, "refresh_rect") and self.hit(sx, sy, self.refresh_rect, pad=30):
                            self.c.refresh_current()

                    if st.get("screen") == "water":
                        if hasattr(self, "open_rect") and self.hit(sx, sy, self.open_rect, pad=35):
                            self.c.servo_open()
                        elif hasattr(self, "close_rect") and self.hit(sx, sy, self.close_rect, pad=35):
                            self.c.servo_close()

                # wacht tot loslaten (voorkomt dubbel klikken)
                t0 = time.time()
                while self.get_touch_raw(samples=3) is not None and time.time() - t0 < 0.5:
                    time.sleep(0.01)
